  ...
  return (
    <div ref={setNodeRef} style={dndStyle} className="group relative rounded-xl overflow-hidden border border-[rgba(255,255,255,0.1)] shadow-[0_2px_8px_rgba(0,0,0,0.2)] hover-lift">
        ...
        <div className="flex items-start gap-2">
          ...
          {allBoards.length > 0 && (
            <button
              ...
              title={t("board.moveToOtherBoard")}
            >
          ...
          >
            ...
            {dueDateObj && (
              <span className={`text-[10px] font-medium ${isOverdue ? "text-red-400" : "text-white/40"}`}>
                {isOverdue ? "⚠ " : "📅 "}
                {dueDateObj.toLocaleDateString(lang === "en" ? "en-US" : "de-DE", { day: "2-digit", month: "short" })}
              </span>
            )}
            ...
            {hasDocAttachment && (
              <span className="flex items-center text-[#3CC79A]" title={card.linkedDoc?.title ?? t("board.docAttached")}>
      ...
      {showMovePicker && allBoards.length > 0 && (
        ...
        >
          <p className="text-xs text-zinc-500 px-3 py-1.5 border-b border-zinc-700">{t("board.moveToBoard")}</p>
          {allBoards.map((b) => (
          ...
          >
            <FontAwesomeIcon icon={faXmark} className="w-3 h-3" /> {t("common.cancel")}
          </button>
        </div>
      )}
    </div>
