  ...
  return (
    <div
      className={`flex flex-col w-72 flex-shrink-0 rounded-xl border transition-colors ${
        isOver ? "border-zinc-500 bg-zinc-800/80" : "border-zinc-700 bg-zinc-900"
      }`}
    >
      {/* Header */}
      <div className="flex items-center gap-2 px-3 pt-3 pb-2 group">
        {editingTitle ? (
          ...
          <h3
            className="flex-1 text-sm font-semibold text-zinc-300 cursor-pointer hover:text-white"
      ...
      <div className="px-3 pb-3">
        {addingCard ? (
          ...
          <button
            ...
            className="w-full text-left text-xs text-zinc-600 hover:text-zinc-300 hover:bg-zinc-800 rounded px-2 py-1 transition-colors"
          >
            + Karte hinzufügen
          </button>
        )}
