...
import {
  ...
  faTrash, faLock, faUsers, faBoxArchive, faFilter, faMagnifyingGlass, faLayerGroup, faRobot, faGear,
...
export function KanbanBoard() {
  ...
  const [archiveBoardId, setArchiveBoardId] = useState<number | null>(null);
  ...
  useEffect(() => {
    if (showArchive && archiveBoardId !== null) void loadArchivedCards(archiveBoardId);
  }, [showArchive, archiveBoardId]);
  ...
  function openArchive() {
    setArchiveBoardId(activeBoardId);
    setShowArchive(true);
  }

  async function loadArchivedCards(boardId: number) {
  ...
  async function saveBoardSettings(id: number, name: string, teamId: number | null) {
    ...
    const data = await res.json();
    if (!res.ok) throw new Error(data.error || "Board konnte nicht gespeichert werden");
    const updated = data as BoardMeta;
    setBoards((previous) => previous.map((item) => item.id === id ? updated : item));
    setBoard((previous) => previous?.id === id ? { ...previous, name: updated.name } : previous);
  }
  ...
  function handleDragStart({ active }: DragStartEvent) {
    ...
    setActiveCard(findCard(id) ?? archivedCards.find((card) => card.id === id) ?? null);
  }
  ...
  async function handleDragEnd({ active, over }: DragEndEvent) {
    ...
    const archivedCard = archivedCards.find((card) => card.id === cardId) ?? null;
    const overCardId = overStr.startsWith("card-") ? Number(overStr.replace("card-", "")) : null;
    const isArchiveTarget = overStr === "archive" || (overCardId !== null && archivedCards.some((card) => card.id === overCardId));

    if (isArchiveTarget) {
      if (!archivedCard) await archiveCard(cardId);
      return;
    }
    ...
    if (overStr.startsWith("col-")) {
      ...
      const targetCardId = Number(overStr.replace("card-", ""));
      const targetCol = findColumnByCardId(targetCardId);
      if (!targetCol) return;
      targetColumnId = targetCol.id;
      const overCard = targetCol.cards.find((c) => c.id === targetCardId);
      newPosition = overCard?.position ?? 0;
    }

    if (archivedCard) {
      await restoreArchivedCard(cardId, targetColumnId, newPosition);
      return;
    }
  ...
  async function archiveCard(id: number) {
    const res = await fetch("/api/board/archive", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ cardId: id }),
    });
    if (!res.ok) return;
    const archivedCard: Card = await res.json();
    ...
    setBoard((prev) => {
      ...
      return {
        ...
        columns: prev.columns.map((col) => ({
          ...col,
          cards: col.cards.filter((card) => card.id !== id).map((card, position) => ({ ...card, position })),
        })),
      };
    });
    if (archiveBoardId === archivedCard.column?.board?.id) {
      setArchivedCards((previous) => [archivedCard, ...previous.filter((card) => card.id !== id)]);
    }
  }

  async function restoreArchivedCard(cardId: number, targetColumnId: number, newPosition: number) {
    const res = await fetch("/api/board/archive", {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ cardId, targetColumnId, newPosition }),
    ...
    if (!res.ok) return;
    setArchivedCards((previous) => previous.filter((card) => card.id !== cardId));
  ...
  async function deleteArchivedCard(cardId: number) {
    const res = await fetch("/api/board/archive", {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ cardId, permanently: true }),
    });
    if (res.ok) setArchivedCards((previous) => previous.filter((card) => card.id !== cardId));
  }
  ...
  return (
    <div className="flex-1 flex overflow-hidden">
      ...
      <aside className="w-[260px] flex-shrink-0 flex flex-col bg-[rgba(18,18,18,0.6)] backdrop-blur-[30px] backdrop-saturate-[180%] border-r border-[rgba(255,255,255,0.08)]">
        <div className="flex flex-col gap-3 px-5 pt-5 pb-3 border-b border-[rgba(255,255,255,0.08)]">
          ...
          <div className="flex items-center gap-1 rounded-xl border border-white/[0.07] bg-white/[0.025] p-1">
            ...
            <button
              ...
              className="w-8 h-8 flex items-center justify-center rounded-lg text-zinc-600 hover:text-zinc-300 hover:bg-white/[0.07] transition-colors disabled:cursor-not-allowed disabled:opacity-30"
              title={t("board.settings.title")}
              aria-label={t("board.settings.title")}
            >
              <FontAwesomeIcon icon={faGear} className="w-3.5 h-3.5" />
            </button>
            <button type="button" onClick={openArchive} className="w-8 h-8 flex items-center justify-center rounded-lg text-zinc-600 hover:text-yellow-500 hover:bg-white/[0.07] transition-colors" title={t("board.archive")}>
              <FontAwesomeIcon icon={faBoxArchive} className="w-3.5 h-3.5" />
            </button>
            <button type="button" onClick={() => setShowTemplateModal(true)} className="w-8 h-8 flex items-center justify-center rounded-lg text-zinc-600 hover:text-[#3CC79A] hover:bg-white/[0.07] transition-colors" title={t("board.templates")}>
              <FontAwesomeIcon icon={faLayerGroup} className="w-3.5 h-3.5" />
            </button>
            <button
              type="button"
              onClick={() => setShowAi(v => !v)}
              className={`w-8 h-8 flex items-center justify-center rounded-lg transition-colors ${showAi ? "text-[#3CC79A] bg-white/[0.08]" : "text-zinc-600 hover:text-[#3CC79A] hover:bg-white/[0.07]"}`}
              title={t("docs.aiAssistant")}
            >
          ...
          <div className="px-3 py-2 border-b border-zinc-800 flex flex-col gap-1.5">
            ...
            <div className="flex gap-1">
              <ThemedSelect
                ...
                onChange={setNewBoardTeamId}
                ariaLabel={t("board.settings.scope")}
                compact
        ...
        <nav className="flex-1 overflow-y-auto py-1">
          ...
          {privateBoards.length > 0 && (
            <div>
              <div className="flex items-center gap-1.5 px-3 py-1 mt-1">
                ...
                <span className="text-[10px] font-semibold text-zinc-700 uppercase tracking-wider">{t("common.private")}</span>
              </div>
          ...
          {boards.length === 0 && (
            <p className="text-xs text-zinc-700 px-3 py-4 text-center">{t("board.noBoards")}</p>
          )}
        </nav>
      </aside>
      ...
      {!board ? (
        <div className="flex-1 flex flex-col gap-3 items-center justify-center text-zinc-600">
          <span>{loadError || (boards.length === 0 ? t("board.noBoards") : t("common.loading"))}</span>
          {loadError && <button onClick={loadBoards} className="rounded-lg bg-zinc-800 px-3 py-1.5 text-xs text-zinc-300 hover:bg-zinc-700">{t("board.retry")}</button>}
        </div>
      ) : (
        <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
            ...
            <div className="flex items-center gap-2 px-6 pt-4 pb-2 flex-shrink-0 flex-wrap">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={t("board.cardSearchPlaceholder")}
              ...
              <div className="relative">
                ...
                {showFilterPanel && (
                  <div className="absolute left-0 top-full mt-1 z-40 bg-[rgba(28,28,32,0.98)] border border-[rgba(255,255,255,0.1)] rounded-xl shadow-2xl p-4 w-64 flex flex-col gap-4">
                    ...
                    {/* Assignee */}
                    <div>
                      <p className="text-[10px] font-semibold uppercase tracking-wider text-white/30 mb-1.5">{t("board.assigned")}</p>
                      <select
                    ...
                    {/* Priority */}
                    <div>
                      <p className="text-[10px] font-semibold uppercase tracking-wider text-white/30 mb-1.5">{t("board.priorityLabel")}</p>
                    ...
                    {/* Due date */}
                    <div>
                      <p className="text-[10px] font-semibold uppercase tracking-wider text-white/30 mb-1.5">{t("board.due")}</p>
                      <div className="flex flex-wrap gap-1.5">
                        {(["overdue", "today", "week"] as const).map((k) => {
                          const labels = { overdue: `⚠ ${t("board.overdue")}`, today: t("planner.today"), week: t("board.thisWeek") };
                          return (
                            <button
                              key={k}
              ...
              {filter.priority !== null && (
                <span className="flex items-center gap-1 text-[10px] rounded-full px-2 py-0.5" style={{ background: PRIORITY_CONFIG[filter.priority as keyof typeof PRIORITY_CONFIG].bg, color: PRIORITY_CONFIG[filter.priority as keyof typeof PRIORITY_CONFIG].color }}>
                  {t(`board.priority.${filter.priority}`)}
              ...
              {filter.due !== null && (
                <span className="flex items-center gap-1 text-[10px] bg-red-500/10 text-red-400 rounded-full px-2 py-0.5">
                  {{ overdue: `⚠ ${t("board.overdue")}`, today: `📅 ${t("planner.today")}`, week: `📅 ${t("board.thisWeek")}` }[filter.due]}
              ...
              {isFiltered && (() => {
                ...
                return hidden > 0 ? (
                  <span className="text-[10px] text-white/30 ml-auto">{t("board.hiddenCards", { count: hidden })}</span>
                ) : null;
              })()}
            </div>
          ...
          <div className="flex-1 flex gap-4 overflow-x-auto px-6 pb-6 items-start">
            ...
            {showArchive && (
              <ArchivePanel
                boards={boards}
                selectedBoardId={archiveBoardId}
                cards={archivedCards}
                loading={archiveLoading}
                onBoardChange={setArchiveBoardId}
                onClose={() => setShowArchive(false)}
                onDelete={deleteArchivedCard}
              />
            )}
            ...
            <div className="w-[320px] flex-shrink-0">
              {addingCol ? (
                <div className="flex flex-col gap-2 bg-[rgba(28,28,28,0.7)] border border-[rgba(255,255,255,0.1)] rounded-2xl p-3">
                  ...
                  />
                  <div className="flex gap-1">
                    <button onClick={addColumn} className="flex-1 bg-blue-600 hover:bg-blue-500 text-white text-xs rounded py-1 font-medium transition-colors">{t("common.add")}</button>
      ...
      {showBoardSettings && activeBoardId !== null && (() => {
        ...
        return (
          <BoardSettingsModal
            ...
            onSave={(name, teamId) => saveBoardSettings(activeBoard.id, name, teamId)}
            onDelete={async () => {
              const res = await fetch(`/api/board/${activeBoard.id}`, { method: "DELETE" });
              const data = await res.json();
              if (!res.ok) throw new Error(data.error || "Board konnte nicht gelöscht werden");
              const remaining = boards.filter((item) => item.id !== activeBoard.id);
              setBoards(remaining);
