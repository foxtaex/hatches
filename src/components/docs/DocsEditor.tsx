...
export function DocsEditor() {
  ...
  async function loadDocs() {
    ...
    try {
      ...
      setLoadError(error instanceof Error ? error.message : t("docs.loadFailed"));
    }
  }
  ...
  async function createDoc() {
    ...
    const docTitle = createTitle.trim() || t("docs.newDoc");
    setCreating(false); setCreateTitle(""); setCreateTeamId("");
    const res = await fetch("/api/docs", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ title: docTitle, teamId }),
    });
  ...
  async function importMarkdown(file: File, teamId: string | null) {
    ...
    if (!res.ok) { alert(t("docs.importFailed")); return; }
    const doc: Doc = await res.json();
    setDocs((prev) => [doc, ...prev]);
    openDoc(doc);
    setImporting(false);
    ...
    <div className="flex-1 flex overflow-hidden" data-color-mode="dark">
      ...
      <aside className="w-[280px] flex-shrink-0 flex flex-col border-r border-[rgba(255,255,255,0.08)] bg-[rgba(18,18,18,0.6)] backdrop-blur-[30px] backdrop-saturate-[180%]">
          ...
          {importing && (
            <div className="flex flex-col gap-2 bg-zinc-800 rounded-lg p-3 border border-zinc-700">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-zinc-300">{t("docs.import")}</span>
        ...
        <nav className="flex-1 overflow-y-auto py-1">
          ...
          {docs.length === 0 && (
            <p className="text-xs text-zinc-700 px-3 py-4 text-center">{t("docs.noDocs")}</p>
          )}
        </nav>
      </aside>
      ...
      {activeId !== null ? (
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="flex items-center gap-3 px-6 py-3 border-b border-[rgba(255,255,255,0.08)] bg-[rgba(24,24,27,0.8)]">
            ...
            {!activeDoc?.teamId && (
              <span className="text-xs text-zinc-600 flex items-center gap-1 flex-shrink-0">
                <FontAwesomeIcon icon={faLock} className="w-2.5 h-2.5" /> {t("common.private")}
              </span>
            )}
            <button
          ...
          <div className="flex-1 overflow-hidden">
            <MarkdownEditor
              ...
              placeholder={t("docs.markdownPlaceholder")}
        ...
        <div className="flex-1 flex flex-col gap-3 items-center justify-center text-zinc-700">
          <span>{loadError || t("docs.noDoc")}</span>
          {loadError && <button onClick={loadDocs} className="rounded-lg bg-zinc-800 px-3 py-1.5 text-xs text-zinc-300 hover:bg-zinc-700">{t("docs.retry")}</button>}
        </div>
      )}
