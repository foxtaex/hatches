...
const QUICK_ACTIONS_EN: Record<AiContext, { label: string; prompt: string }[]> = {
  docs: [
    { label: "Write a section", prompt: "Write the next section based on the existing content." },
    { label: "Summarize", prompt: "Summarize the current content in 3–5 sentences." },
    { label: "Improve", prompt: "Improve the writing style without changing the meaning." },
    { label: "Create outline", prompt: "Create a structured Markdown outline for this document." },
  ],
  board: [
    { label: "Plan sprint", prompt: "Suggest a sprint board structure with columns and typical task cards." },
    { label: "Generate cards", prompt: "Generate five concrete task cards with titles and short descriptions for this board." },
    { label: "Expand task", prompt: "Write a detailed task description in Markdown format." },
    { label: "Retrospective", prompt: "Create questions for a sprint retrospective covering wins, improvements, and actions." },
  ],
};
...
export function AiAssistant({ context, contextData, onInsertText, onClose }: Props) {
  ...
  const send = useCallback(async (text: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        role: "assistant",
        content: data.error ?? data.content ?? t("ai.noAnswer"),
        timestamp: new Date(),
        error: !!data.error,
      }]);
    } catch {
  ...
  }, [messages, loading, context, contextData, t]);
  ...
  const quickActions = (lang === "en" ? QUICK_ACTIONS_EN : QUICK_ACTIONS)[context];
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3">
        ...
        {loading && (
          <div className="flex items-center gap-2 px-3 py-2.5">
            ...
            <span className="text-xs" style={{ color: "rgba(255,255,255,0.35)" }}>{t("ai.generating")}</span>
          </div>
        )}
